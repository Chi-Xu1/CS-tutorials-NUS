...
public class AVLTree<T extends Comparable<T>> {
    ...
    private boolean insert(Node<T> node, T key, Node<T> parent) {
        if (node == null) {
            node = new Node<T>(key, null, null, parent);
            return true;
        }
        Node<T> cur = node;
        Node<T> p = parent;
        ...
        if (left) {
            p.left = new Node<T>(key, null, null, p);
        } else {
            p.right = new Node<T>(key, null, null, p);
        }

        while (p != null) {
            refreshHeight(p);
            if (Math.abs(height(p.left) - height(p.right)) > MAX_HEIGHT_DIFFERENCE) {
                p = balance(p);
                refreshHeight(p);
                refreshWeight(p);
            }
        ...
        // if (node == null) {
        //     node = new Node<>(key, null, null, parent);
        // }
    ...
    private T getKeyByRank(Node<T> node, int rank) {
        if (rank == weight(node.left) + 1) {
            return node.key;
        } else {
            if (rank < weight(node.left) + 1) {
                return getKeyByRank(node.left, rank);
            } else {
                return getKeyByRank(node.right, rank - (weight(node.left) + 1));
            }
        }
    }
}
